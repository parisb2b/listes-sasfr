...
6. **Règle de non-régression, valable pour ce prompt et pour tout prompt de correction futur sur ce projet** : ne jamais modifier, réécrire, renommer ou "nettoyer" du code qui n'est pas directement concerné par la tâche demandée. Toute correction ou ajout futur sur ce projet doit rester strictement chirurgical : identifier précisément le ou les fichiers/fonctions concernés, ne toucher qu'à cela, et vérifier après coup que les fonctionnalités déjà construites continuent de fonctionner. En cas de doute sur l'étendue d'une modification, toujours choisir l'option la plus limitée plutôt que la plus "propre" ou la plus complète.
